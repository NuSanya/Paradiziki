...
/obj/item/toy/plushie/kotrazumist/attack_self(mob/user)
	...
	user.visible_message("[icon2html(src, viewers(src))] [span_notice(razumisttext)]")
...
/obj/item/toy/plushie/kotwithfunnyhat/attack_self(mob/user)
	...
	user.visible_message("[icon2html(src, viewers(src))] [span_notice(ricetext)]")
	COOLDOWN_START(src, cooldown, 3 SECONDS)
...
/obj/item/toy/plushie/rdplushie/proc/interaction(mob/user)
	...
	user.visible_message("[icon2html(src, viewers(src))] [span_notice(message)]")
	COOLDOWN_START(src, cooldown, 3 SECONDS)
...
/obj/item/toy/plushie/gsbplushie/proc/interaction(mob/user)
	...
	visible_message("[icon2html(src, viewers(src))] [span_notice(message)]")
	COOLDOWN_START(src, cooldown, 3 SECONDS)
...
/obj/item/toy/plushie/greyplushie/water_act(volume, temperature, source, method = REAGENT_TOUCH) //If water touches the plushie the following code executes.
	...
	src.visible_message("[icon2html(src, viewers(src))] [span_danger("AAAAAAХ!")]")
	if(singed)
		return
	singed = TRUE
...
/obj/item/toy/plushie/greyplushie/attack_self(mob/user)//code for talking when hugged.
	...
	if(singed)//If the plushie is water damaged it'll say Ow instead of talking in wingdings.
		visible_message("[icon2html(src, viewers(src))] [span_danger("Ow...")]")
	else//If the plushie has not touched water they'll say Greetings in wingdings.
		visible_message("[icon2html(src, viewers(src))] [span_danger("☝︎❒︎♏︎♏︎⧫︎♓︎■︎♑︎⬧︎📬︎")]")

/obj/item/toy/plushie/voxplushie/attack_self(mob/user)
	if(!cooldown)
		playsound(user, 'sound/voice/shriek1.ogg', 10, FALSE)
		user.visible_message("[icon2html(src, viewers(src))] [span_danger("Skreee!")]")
		cooldown = 1
		spawn(30) cooldown = 0
		return
	..()
...
/obj/item/toy/plushie/shardplushie/attack_self(mob/user)
	...
	user.visible_message("[icon2html(src, viewers(src))] [span_danger("ДЕСТАБИЛИЗАЦИЯ!")]")
	cooldown = TRUE
...
/obj/item/toy/plushie/ashwalkerplushie/attack_self(mob/user)
	...
	switch(rand(1, 20))
		if(1 to 12)
			...
			user.visible_message("[icon2html(src, viewers(src))] [span_danger("Hsss!")]")
		if(13 to 19)
			...
			user.visible_message("[icon2html(src, viewers(src))] [span_danger("RAAAAAWR!")]")
		if(20)
			playsound(src, pick('sound/voice/unathi/rumble.ogg', 'sound/voice/unathi/rumble2.ogg'), 40, 1)
			user.visible_message("[icon2html(src, viewers(src))] [span_notice("Пеплоходец выглядит расслабленным.")]")
	cooldown = TRUE
	addtimer(VARSET_CALLBACK(src, cooldown, FALSE), 3 SECONDS)
...
/obj/item/toy/plushie/nianplushie/attack_self(mob/user)
	...
	user.visible_message("[icon2html(src, viewers(src))] [span_danger("Buzzzz!")]")
	cooldown = TRUE
	addtimer(VARSET_CALLBACK(src, cooldown, FALSE), 3 SECONDS)
...
/obj/item/toy/plushie/ninja/attack_self(mob/user as mob)
	...
	if(cooldown < world.time)
		...
		switch(plushie_color)
			if("green")
				user.visible_message(span_notice("[icon2html(src, viewers(src))] [capitalize(declent_ru(NOMINATIVE))] говорит: \"Я не боюсь тьмы! Я сама тьма!\""))
			if("blue")
				user.visible_message(span_notice("[icon2html(src, viewers(src))] [capitalize(declent_ru(NOMINATIVE))] говорит: \"Твой жалкий свет меня не остановит!\""))
			if("red")
				user.visible_message(span_notice("[icon2html(src, viewers(src))] [capitalize(declent_ru(NOMINATIVE))] говорит: \"Ты можешь бежать, но не сможешь спрятаться!\""))
		plushie_color = null
...
/obj/item/toy/plushie/realgoat/attack_self(mob/user)
	...
	user.visible_message("[icon2html(src, viewers(src))] [span_danger("Baaaaah!")]")
	cooldown = TRUE
	addtimer(VARSET_CALLBACK(src, cooldown, FALSE), 3 SECONDS)
...
/obj/item/toy/plushie/blahaj/twohanded/attack_self(mob/user)
	...
	user.visible_message("[icon2html(src, viewers(src))] [span_boldnotice("Rawr!")]")
	cooldown = TRUE
	addtimer(VARSET_CALLBACK(src, cooldown, FALSE), 3 SECONDS)
...
/obj/item/toy/plushie/axolotlplushie/attack_self(mob/user)
	...
	user.visible_message("[icon2html(src, viewers(src))] [span_danger("Squeeek!")]")
	cooldown = TRUE
	addtimer(VARSET_CALLBACK(src, cooldown, FALSE), 3 SECONDS)
...
/obj/item/toy/plushie/plasmamanplushie/attack_self(mob/user)
	...
	user.visible_message("[icon2html(src, viewers(src))] [span_danger("Плазззма Вечна!")]")
	cooldown = TRUE
	addtimer(VARSET_CALLBACK(src, cooldown, FALSE), 3 SECONDS)
...
/obj/item/toy/plushie/rouny/attack_self(mob/user)
	...
	user.visible_message("[icon2html(src, viewers(src))] [span_danger("Бежиииииим!")]")
	cooldown = TRUE
	addtimer(VARSET_CALLBACK(src, cooldown, FALSE), 3 SECONDS)
...
/obj/item/toy/plushie/bubblegumplushie/attack_self(mob/user)
	...
	user.visible_message("[icon2html(src, viewers(src))] [span_danger("Бубльгум топает...")]")
	cooldown = TRUE
	addtimer(VARSET_CALLBACK(src, cooldown, FALSE), 3 SECONDS)
...
/obj/item/toy/AI/attack_self(mob/user)
	if(!cooldown) //for the sanity of everyone
		...
		user.visible_message(span_danger("[icon2html(src, viewers(src))] [message]"))
		cooldown = 1
		spawn(30) cooldown = 0
		return
	..()
...
/obj/item/toy/codex_gigas/attack_self(mob/user)
	...
	for(var/message in messages)
		user.loc.visible_message(span_danger("[icon2html(src, viewers(src))] [message]"))
		sleep(1 SECONDS)
...
/obj/item/toy/figure/attack_self(mob/user as mob)
	if(cooldown < world.time)
		cooldown = (world.time + 30) //3 second cooldown
		user.visible_message(span_notice("[icon2html(src, viewers(src))] [capitalize(declent_ru(NOMINATIVE))] говорит \"[toysay]\"."))
		playsound(user, 'sound/machines/click.ogg', 20, TRUE)
...
/obj/item/toy/eight_ball/attack_self(mob/user as mob)
	if(!cooldown)
		...
		user.visible_message(span_notice("[icon2html(src, viewers(src))] [capitalize(declent_ru(NOMINATIVE))] говорит: \"[answer]\""))
		spawn(30)
			cooldown = 0
		return
