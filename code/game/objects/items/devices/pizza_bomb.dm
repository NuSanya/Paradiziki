...
/obj/item/pizza_bomb/proc/go_boom()
	if(disarmed)
		visible_message(span_danger("[icon2html(src, viewers(src))] Sparks briefly jump out of the [correct_wire] wire on  [src], but it's disarmed!"))
		return
...
/obj/item/pizza_bomb/wirecutter_act(mob/living/user, obj/item/I)
	...
	if(chosen_wire != correct_wire)
		...
		go_boom()
		return .
	audible_message(span_warning("[icon2html(src, hearers(src))] The [name] suddenly stops beeping and seems lifeless."))
