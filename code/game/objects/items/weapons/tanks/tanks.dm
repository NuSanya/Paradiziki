...
/obj/item/tank/examine(mob/user, show_contents_info = TRUE)
	...
	if(!in_range(src, user))
		if(icon == src)
			. += span_notice("It's \a [icon2base64html(icon)][src]! If you want any more information you'll need to get closer.")
		return
	...
	. += span_notice("\The [icon2base64html(icon)][src] feels [descriptive]")
