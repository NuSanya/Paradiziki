...
/obj/item/pneumatic_cannon/examine(mob/user)
	...
	if(!in_range(user, src))
		...
		if(tank)
			. += span_notice("[icon2base64html(tank)] It has [tank] mounted onto it. It could be removed with a <b>screwdriver</b>.")
		for(var/obj/item/I in loadedItems)
			. += span_notice("[icon2base64html(I)] It has \a [I] loaded.")
