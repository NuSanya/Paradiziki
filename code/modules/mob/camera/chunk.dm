...
/datum/camerachunk/proc/add_camera(obj/machinery/camera/cam)
	...
	RegisterSignal(cam, COMSIG_QDELETING, PROC_REF(remove_camera), TRUE)
...
/datum/camerachunk/proc/add(mob/camera/eye/eye, add_images = TRUE)
	...
	seenby += eye
	RegisterSignal(eye, COMSIG_QDELETING, PROC_REF(eye_destroyed))
	if(changed)
		SScamera.queue(src)
...
/datum/camerachunk/proc/update()
	...
	for(var/obj/machinery/camera/current_camera as anything in active_cameras)
		if(!current_camera || !current_camera.can_use() || !current_camera.can_AI_see())
			continue

		var/turf/point = locate(src.x + (CAMERA_CHUNK_SIZE / 2), src.y + (CAMERA_CHUNK_SIZE / 2))
		if(get_dist(point, current_camera) > CAMERA_CHUNK_SIZE + (CAMERA_CHUNK_SIZE / 2))
			continue

		for(var/turf/vis_turf in current_camera.camera_see())
			if(turfs[vis_turf])
				new_visible_turfs[vis_turf] = vis_turf
...
/datum/camerachunk/New(loc, x, y, lower_z)
	...
	for(var/z_level in lower_z to upper_z)
		for(var/obj/machinery/camera/c in urange(half_chunk + CAMERA_VIEW_DISTANCE, locate(x + half_chunk, y + half_chunk, z), z_level))
			add_camera(c)

		for(var/turf/t in block(max(x, 1), max(y, 1), z_level, min(x + CAMERA_CHUNK_SIZE - 1, world.maxx), min(y + CAMERA_CHUNK_SIZE - 1, world.maxy), z_level))
			turfs[t] = t
